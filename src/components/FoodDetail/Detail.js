...
function Detail() {
  ...
  const rating = []
  for (let index = recipe.rating; index > 0; index--) {
    if(index === 0.5){
      rating.push(<svg
  ...
  return (
    <>
      <div className="h-full px-8 pb-4 bg-gray-200">
        <div className="flex gap-4 py-8 border-b-2 border-gray-400">
          <div className="w-60 h-60 ">
            <img
              className="rounded-lg w-full h-full"
              src={recipe?.image_url ? require("" + recipe?.image_url) : ""}
              alt={recipe.name}
            />
          </div>
          <div className="w-3/5">
            <h1 className="text-4xl text-gray-700 font-bold">{recipe.name}</h1>
          ...
          <div className="flex flex-wrap flex-col bg-gray-100 rounded-md w-60 h-60 p-4 items-center">
            ...
            <h1 className="font-bold text-4xl">{recipe?.rating ?recipe?.rating +"/5":"No Reviews"}</h1>
            <span className="text-gray-400 mt-6">{recipe?.rating ?"12,965 Reviews":"No Reviews"}</span>
          </div>
        </div>
        <div className="mt-4 grid grid-cols-4">
          <div>
            ...
            {recipe.ingredients?.map((data) => {
              return <h1 className="font-semibold text-gray-800 text-md">{"- "+data}</h1>;
            })}
